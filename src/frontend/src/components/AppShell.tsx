...
const AGM_PRO_URL = withBase("connected-sites/agm-pro/index.html");
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
        {visibleItems.map((item) => {
          const isActive =
            !item.externalHref && item.to === "/"
              ? location.pathname === "/"
              : !item.externalHref && location.pathname.startsWith(item.to);
          if (item.externalHref) {
            return (
              ...
              >
                <span className="flex-shrink-0 text-foreground/60 group-hover:text-foreground">
                  {item.icon}
                </span>
        ...
        if (item.externalHref) {
          return (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex-1 flex flex-col items-center justify-center gap-0.5 text-[10px] font-medium text-muted-foreground transition-smooth"
              data-ocid={`bottom_nav.${item.label.toLowerCase().replace(/\s+/g, "_")}.link`}
            >
              <span className="p-1 rounded-lg transition-smooth">
                {item.icon}
              </span>
            ...
            if (item.externalHref) {
              return (
                <a
                  ...
                  rel="noreferrer"
                  onClick={onClose}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-foreground/70 hover:bg-muted/60 hover:text-foreground transition-smooth"
                >
                  {item.icon}
            ...
            if (item.externalHref) {
              return (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center justify-center gap-2 min-h-11 px-2 xl:px-3 rounded-lg text-[12px] xl:text-sm font-semibold whitespace-nowrap text-foreground/70 hover:bg-muted/60 hover:text-foreground transition-smooth"
